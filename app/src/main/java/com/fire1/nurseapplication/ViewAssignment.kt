...
import com.fire1.nurseapplication.adapters.ViewAssignmentsAdapter
...
class ViewAssignment : AppCompatActivity() {
    lateinit var itemList: List<Nurse_Assignment>
    lateinit var viewAssignmentsAdapter: ViewAssignmentsAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewAssignmentsAdapter = ViewAssignmentsAdapter(applicationContext)
        ...
        val changePassword = findViewById<MaterialButton>(R.id.change_pass)
    ...
    fun post_fetch(){
        val api = Constants.BASE_URL+"/view_assignments"
        ...
        body.put("nurse_id", "2") //NB: 4 is static
        body.put("flag", "active")
        val helper = ApiHelper(applicationContext)
        helper.post(api, body, object : ApiHelper.CallBack{
            override fun onSuccess(result: JSONArray?) {
                val gson = GsonBuilder().create()
                itemList = gson.fromJson(result.toString(),
                    Array<Nurse_Assignment>::class.java).toList()
                //Finally, our adapter has the data
                viewAssignmentsAdapter.setListItems(itemList)
                //For the sake of recycling/Looping items, add the adapter to recycler
                recyclerView.adapter = viewAssignmentsAdapter
    ...
    private fun filter(text: String) {
        ...
        val filteredlist: ArrayList<Nurse_Assignment> = ArrayList()
        ...
        if (filteredlist.isEmpty()) {
            ...
            viewAssignmentsAdapter.filterList(filteredlist)
        } else {
